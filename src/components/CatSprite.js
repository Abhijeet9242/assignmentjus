...
export default function CatSprite() {
  ...
  const {deg,leftstatus,rightstatus} = useContext(RotationContext)
  const {dis,disstatus} = useContext(RotationContext)
  ...
  return (
    ...
    <svg
      ...
      xmlSpace="preserve"
      id = "my"
      transform = {leftstatus ? `rotate(${deg})` : rightstatus ? `rotate(${deg})` : disstatus ? `translate(${dis})` : "rotate(0)" }
